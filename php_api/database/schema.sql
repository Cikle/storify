...
CREATE TABLE IF NOT EXISTS `items` (
  ...
  `expiry_date`        DATE          NULL DEFAULT NULL COMMENT 'Expiration date (optional)',
  `unit`               VARCHAR(50)   NULL DEFAULT NULL COMMENT 'Gramm | Packung | Rollen | Stück | Flaschen | Dosen | Tuben',
  `critical_threshold` INT           NULL DEFAULT NULL COMMENT 'Per-item low-stock threshold override',
  `warning_days`       INT           NULL DEFAULT NULL COMMENT 'Days before expiry to warn (overrides global 7)',
  `pack_size`          INT           NULL DEFAULT 1   COMMENT 'Sub-units per pack (e.g. 12 rolls per Packung)',
  `photo_url`          VARCHAR(500)  NULL DEFAULT NULL COMMENT 'Relative path to uploaded photo',
  PRIMARY KEY (`id`),
